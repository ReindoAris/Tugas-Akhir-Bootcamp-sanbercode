...
import loginPage from "./POM_Directory.cy";
...
describe('Login Feature',() => {
    it('User Login with Valid credentials',() => {
        ...
        loginPage.menuDirectory().contains('Directory').click();
        loginPage.textDirectory().should('have.text','Directory');
        loginPage.inputemployee().type('rere');
        loginPage.selectname().contains('yaya').click();
        //loginPage.selectJobtitle().contains('select').click();
        loginPage.jobTitle().eq(0).click();
        loginPage.selectJobtitle().contains('HR Manager').click();
        loginPage.location().eq(1).click();
        loginPage.selectlocation().contains('Texas R&D').click();
